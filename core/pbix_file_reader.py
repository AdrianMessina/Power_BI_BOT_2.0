...
_ext_project = Path("c:/Users/SE46958/1 - Claude - Proyecto viz/documentation_generator_v3")


def _load_external_parser():
    """
    Carga PBIXParser desde documentation_generator_v3.

    Estrategia: guardar todos los módulos 'core.*' del bot, removerlos de
    sys.modules, importar el 'core' externo, extraer PBIXParser, y restaurar
    los módulos del bot.
    """
    global PBIXParser, PARSER_AVAILABLE

    if not _ext_project.exists():
        logger.warning(f"Proyecto parser no encontrado: {_ext_project}")
        return

    ext_str = str(_ext_project)

    # 1. Guardar y remover los módulos 'core.*' del bot (excepto este archivo que se está cargando)
    this_module_name = __name__  # 'core.pbix_file_reader'
    stashed = {}
    for name in list(sys.modules.keys()):
        if name == 'core' or name.startswith('core.'):
            if name == this_module_name:
                continue  # No tocar el módulo que se está ejecutando
            stashed[name] = sys.modules.pop(name)

    # 2. Agregar proyecto externo al inicio del path
    sys.path.insert(0, ext_str)

    try:
        import importlib
        ext_core = importlib.import_module('core.parsers.pbix_parser')
        PBIXParser = ext_core.PBIXParser
        PARSER_AVAILABLE = True
        logger.info("Parser externo cargado correctamente")
    except Exception as e:
        logger.warning(f"Parser no disponible: {e}")
    finally:
        # 3. Limpiar módulos externos que se cargaron
        for name in list(sys.modules.keys()):
            if name == 'core' or name.startswith('core.'):
                if name == this_module_name:
                    continue
        ...
        # 4. Restaurar módulos del bot
        sys.modules.update(stashed)

        # 5. Quitar proyecto externo del path
        if ext_str in sys.path:
            sys.path.remove(ext_str)


_load_external_parser()
